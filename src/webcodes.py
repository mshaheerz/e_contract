...
# @login_required
def view_work():
    qry="SELECT `users`.*,`works`.*,`location`.* FROM `users` JOIN `works` ON  `users`.`loginid`=`works`.`loginid` JOIN `location` ON `location`.userid=`users`.loginid GROUP BY `works`.`workid`"
    res=select(qry)
...
@app.route('/cmploc',methods=['post'])
# @login_required
def cmploc():
    uid=session['uid']
    lat=request.form['lat']
    long=request.form['lng']
    qry="INSERT INTO `location` VALUES(NULL,%s,'pending',%s,%s)"
    val=(uid,lat,long)
    iud(qry,val)
    return render_template('user/location.html')
